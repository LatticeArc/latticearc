...
//!
//! This implementation uses the audited `hmac` crate from RustCrypto.
...
/// HMAC-SHA256 type for HKDF operations
...
///
/// # Errors
/// This function does not return errors under normal conditions.
...
pub fn hkdf_extract(salt: Option<&[u8]>, ikm: &[u8]) -> Result<[u8; 32]> {
    ...
    const DEFAULT_SALT: [u8; 32] = [0u8; 32]; // HashLen zeros for SHA-256

    let salt_bytes: &[u8] = match salt {
        Some(s) if !s.is_empty() => s,
        _ => &DEFAULT_SALT, // RFC 5869: default to HashLen zeros when salt is empty or None
    };

    // HKDF-Extract: PRK = HMAC-Hash(salt, IKM)
    // salt is used as the HMAC key, IKM is the data
    let mut mac = HmacSha256::new_from_slice(salt_bytes).map_err(|_e| {
        LatticeArcError::InvalidParameter("HKDF extract: invalid salt".to_string())
    })?;
    mac.update(ikm);
...
pub fn hkdf_expand(prk: &[u8; 32], info: Option<&[u8]>, length: usize) -> Result<HkdfResult> {
    ...
    // RFC 5869 HKDF-Expand implementation using HMAC
    // N = ceil(L/HashLen)
    ...
    for i in 1..=n {
        ...
        let mut mac = HmacSha256::new_from_slice(prk).map_err(|_e| {
            LatticeArcError::InvalidParameter("HKDF expand: invalid PRK".to_string())
        })?;
        mac.update(&t_prev);
