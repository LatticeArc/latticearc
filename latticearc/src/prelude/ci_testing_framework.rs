...
impl PreludeCiReport {
    ...
    pub fn generate_report(&self) -> String {
        ...
        // Compliance Status section: was previously six unconditional
        // ✅ checkmarks emitted regardless of whether unit_tests_passed
        // / property_tests_passed / memory_safety_passed were true.
        // That made the section a misleading-assurance footgun — a
        // consumer reading the bottom of the report would see all-green
        // even when the per-line statuses above said FAILED. Drive each
        // line off the same booleans the per-line summary uses;
        // categories that aren't backed by a boolean (CAVP, side-
        // channel, performance) are now reported as their own
        // "PRESENT" / "ABSENT" status rather than a misleading ✅.
        report.push_str("\n## 🎯 Compliance Status\n\n");
        report.push_str(&format!(
            "- Unit Test Coverage: {}\n",
            if self.unit_tests_passed { "✅ PASSED" } else { "❌ FAILED" }
        ...
        report.push_str(&format!(
            "- CAVP Compliance Report: {}\n",
            if self.cavp_compliance_report.is_some() {
                "✅ ATTACHED"
            } else {
                "⚠️ NOT ATTACHED"
            }
        ...
        report.push_str(&format!(
            "- Side-Channel Analysis Report: {}\n",
            if self.side_channel_report.is_some() { "✅ ATTACHED" } else { "⚠️ NOT ATTACHED" }
        ));
        // Performance benchmarks always run (collected into
        // performance_results), so always present.
        report.push_str("- Performance Benchmarks: ✅ ATTACHED\n");
