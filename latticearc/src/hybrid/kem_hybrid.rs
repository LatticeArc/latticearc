...
impl ConstantTimeEq for HybridKemSecretKey {
    ...
    ///
    /// Compares (security level, ML-KEM decapsulation-key bytes, X25519 seed
    /// bytes) unconditionally via `subtle::ConstantTimeEq` so timing does
    /// not reveal which component differs.
    ///
    /// If byte serialization of either key fails (not reachable for keys
    /// generated by this crate), the corresponding leg returns
    /// `Choice::from(0)` — never a false positive, even if both sides fail
    /// simultaneously. Length mismatch in the ML-KEM leg (different
    /// security levels) likewise returns 0; the preceding `level_match`
    /// check will have already done so, and the resulting wall-clock
    /// divergence reveals only the non-secret security level.
    fn ct_eq(&self, other: &Self) -> subtle::Choice {
        use subtle::Choice;

        // `MlKemSecurityLevel` is not `#[repr(u8)]`, so do not rely on
        // discriminant-cast ordering. `PartialEq` is a total equality on
        // the enum's variants.
        ...
        let ec_match = match (self.ecdh_seed_bytes(), other.ecdh_seed_bytes()) {
            (Ok(a), Ok(b)) => a.ct_eq(&*b),
            _ => Choice::from(0u8),
        };

        level_match & ml_match & ec_match
    }
}
