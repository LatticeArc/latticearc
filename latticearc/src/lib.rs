...
//! | `test-utils` | Exposes adversarial-test mutators on otherwise-immutable proof types (`SigmaProof::challenge_mut`) and FIPS module-error-state recovery helpers (`clear_error_state`, `restore_operational_state`). **Soundness-bypassing** / FIPS 140-3 §9.6 state-machine bypass — must NOT be enabled in production builds. Default-off. |
