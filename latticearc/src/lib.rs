...
//! | `secret-mlock` | OS-level memory locking for `SecretVec` (Secret Type Invariant I-10). Uses `mlock(2)` on Linux/macOS via `region`. **No-op on Windows** — `VirtualLock` is documented as best-effort and `VirtualUnlock`'s `ERROR_NOT_LOCKED` path causes a `panic = "abort"` build to abort the process; the round-17 audit fix gates this feature off on Windows targets so the cfg evaluates `false` regardless of the feature flag. Default-off because `RLIMIT_MEMLOCK` can fail at constructor time on some deployments. |
...
//! | `test-utils` | Exposes FIPS module-error-state recovery helpers (`clear_error_state`, `restore_operational_state`) for negative tests that need to recover global state without re-running POST. FIPS 140-3 §9.6 state-machine bypass — must NOT be enabled in production builds. Default-off. (Note: earlier rounds listed `SigmaProof::challenge_mut` here. Round-35 M3 narrowed it to `#[cfg(test)]` — strictly narrower than `test-utils` — so downstream consumers **cannot** acquire the soundness-bypass mutator by enabling this feature. The mutator is reachable only from in-tree lib unit tests.) |
