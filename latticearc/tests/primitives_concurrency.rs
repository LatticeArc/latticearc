//! Concurrency / thread-safety tests for arc-primitives.
#![deny(unsafe_code)]

// Originally: primitives_concurrency_comprehensive.rs
mod comprehensive {
    #![allow(
        clippy::panic,
        clippy::unwrap_used,
        ...
        let handles: Vec<_> = (0..9)
            .map(|i| {
                ...
                thread::spawn(move || {
                    for _ in 0..5 {
                        let (pk, _sk) =
                            ml_dsa::generate_keypair(param).expect("keygen should succeed");
                        let mut keys_guard = keys.lock().expect("mutex should not be poisoned");
        ...
        let handles: Vec<_> = (0..STANDARD_THREAD_COUNT)
            .map(|_| {
                ...
                thread::spawn(move || {
                    for _ in 0..STANDARD_ITERATIONS {
                        let alice =
                            EcdhP256KeyPair::generate().expect("alice keygen should succeed");
        ...
        let handles: Vec<_> = (0..4)
            .map(|_| {
                ...
                thread::spawn(move || {
                    for _ in 0..5 {
                        let alice =
                            EcdhP384KeyPair::generate().expect("alice keygen should succeed");
        ...
        let handles: Vec<_> = (0..4)
            .map(|_| {
                ...
                thread::spawn(move || {
                    for _ in 0..3 {
                        let alice =
                            EcdhP521KeyPair::generate().expect("alice keygen should succeed");
            ...
            .map(|_| {
                ...
                thread::spawn(move || {
                    for _ in 0..STANDARD_ITERATIONS * 5 {
                        ...
                        let mut values =
                            random_values.lock().expect("mutex should not be poisoned");
                        values.push(bytes);
                    }
                })
            })
                ...
                thread::spawn(move || {
                    for _ in 0..STANDARD_ITERATIONS * 5 {
                        ...
                        if let Ok(restored_pk) =
                            latticearc::primitives::kem::ml_kem::MlKemPublicKey::from_bytes(
                                &pk_bytes,
                                MlKemSecurityLevel::MlKem768,
                            )
                        {
            ...
            .map(|_| {
                ...
                thread::spawn(move || {
                    for _ in 0..STANDARD_ITERATIONS * 3 {
                        ...
                        {
                            let (_pk2, _sk2) =
                                MlKem::generate_keypair(MlKemSecurityLevel::MlKem768)
                                    .expect("keygen should succeed");
    ...
    fn test_comprehensive_concurrency_summary_succeeds() {
        ...
        let nonce = AesGcm256::generate_nonce();
        let (ct, tag) =
            cipher.encrypt(&nonce, b"test", None).expect("AES-GCM encrypt should succeed");
...
// Originally: primitives_concurrency_tests.rs
mod basic {
    ...
    #[test]
    fn test_parallel_keygen_produces_unique_keys_are_unique() {
    ...
    #[test]
    fn test_concurrent_keygen_different_security_levels_succeeds() {
        let success_count = Arc::new(AtomicUsize::new(0));
        let levels = [
            MlKemSecurityLevel::MlKem512,
            MlKemSecurityLevel::MlKem768,
    ...
    #[test]
    fn test_concurrent_encapsulation_same_key_succeeds() {
    ...
    #[test]
    fn test_concurrent_different_ciphertexts_are_unique() {
        let (pk, _sk) = MlKem::generate_keypair(MlKemSecurityLevel::MlKem512)
            .expect("keypair generation should succeed");
    ...
    #[test]
    fn test_thread_local_rng_isolation_succeeds() {
        ...
        let secrets = shared_secrets.lock().expect("mutex should not be poisoned");
        let mut unique_secrets: std::collections::HashSet<Vec<u8>> =
    ...
    #[test]
    fn test_high_concurrency_stress_succeeds() {
    ...
    #[test]
    fn test_no_data_races_during_operation_succeeds() {
            ...
            .map(|_| {
                thread::spawn(move || {
                    for _ in 0..OPS_PER_THREAD {
                        let (pk, sk) =
                            MlKem::generate_keypair(MlKemSecurityLevel::MlKem768).expect("keygen");
                        let (_ss, ct) = MlKem::encapsulate(&pk).expect("encap");
