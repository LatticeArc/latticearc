...
2. **`scripts/ci/secret_type_audit.sh`** — wired into `.github/workflows/security.yml` as the `secret-type-invariants` job. Finds every struct whose name matches `(Secret|Private|Signing|Keypair|KeyPair)` and asserts, **per-struct**:
   - **I-1**: `#[derive(...Zeroize...)]` or `#[derive(...ZeroizeOnDrop...)]` on the struct, OR an explicit `impl Zeroize for THIS_STRUCT` / `impl ZeroizeOnDrop for THIS_STRUCT` in the same file, OR a struct field whose type matches a known secret-wrapping type (`SecretBytes`, `SecretVec`, `PrivateKey`, `SymmetricKey`, `MlKemSecretKey`, `EphemeralPrivateKey`, `DecapsulationKey`, `Zeroizing<_>`). Transitive composition via these wrappers satisfies the wipe-on-drop invariant without requiring an explicit re-derive.
   ...
   - **I-4**: an `impl` block whose target type is THIS_STRUCT (inherent `impl NAME { ... }` or trait `impl TRAIT for NAME { ... }`) defines a method whose name matches `expose_[a-z_]*secret` (covers single-secret holders with `expose_secret` and hybrid multi-secret types with `expose_ml_dsa_secret` / `expose_ed25519_secret`). A method declared on a *different* struct in the same file does not satisfy this invariant. `KeyPair`/`Keypair` composition wrappers are exempt — secret material is reached through inner-field accessors that each carry their own sealed accessor.

   All four checks are scoped to the specific struct under audit. An earlier revision used file-wide `grep -qE` for I-1 and I-4, which let a single Zeroize impl or `expose_secret` accessor anywhere in a file pass every other secret struct in that file (audit finding M-NEW-1). The CI job runs `--self-test` first, which plants synthetic file-wide-grep reproducers and fails the build if the gate ever regresses to that anti-pattern.
