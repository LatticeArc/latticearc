...
| `primitives::sig::ml_dsa` | **Gap**: `generate_keypair`, `sign`, `verify` do not call `validate_signature_size`. Convenience layer (`convenience::pq_sig`) catches this, but a caller reaching directly into `primitives::sig::ml_dsa::sign(message, ...)` bypasses the cap. |
