...
| `hybrid::pq_only` | `encrypt_pq_only`, `decrypt_pq_only`, `encrypt_pq_only_with_aad`, `decrypt_pq_only_with_aad` | **Gap**: same as `encrypt_hybrid`. The `_with_aad` siblings (added in a later audit-fix wave) share internals with the base versions; the underlying AEAD primitive enforces the cap. |
...
| `hybrid::kem_hybrid` | `generate_keypair*`, `encapsulate`, `decapsulate`, `decapsulate_from_parts` | N/A — ML-KEM fixed-size inputs. The `decapsulate_from_parts` entry point (added in a later audit-fix wave to remove the placeholder `EncapsulatedKey` hack) takes raw `&[u8]` slices but `MlKemCiphertext::new` validates the length against the parameter set's fixed ciphertext size before any work runs. |
