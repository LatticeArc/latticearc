...
fuzz_target!(|data: &[u8]| {
    ...
    let fuzzed_ct = HybridCiphertext::new(
        data.get(..1088).unwrap_or(&[]).to_vec(),
        vec![],
        data.get(1088..1100.min(data.len())).unwrap_or(&[]).to_vec(),
        data.get(..12).unwrap_or(&[]).to_vec(),
        data.get(..16).unwrap_or(&[]).to_vec(),
    );
    ...
    if let Ok(mut ct) = encrypt(pk.as_bytes(), plaintext, None) {
    ...
    if data.len() >= 10000 {
        let large_ct = HybridCiphertext::new(
            vec![0u8; 1088],
            vec![],
            data[..10000].to_vec(),
            vec![0u8; 12],
