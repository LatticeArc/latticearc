...
use arc_core::types::{CryptoConfig, SecurityLevel, UseCase};
...
fn test_arc_core_init_with_default_config() {
    let config = arc_core::config::CoreConfig::default();
...
fn test_self_tests_passed() {
    ...
    let _ = arc_core::init();
    assert!(arc_core::self_tests_passed(), "Self-tests should have passed after init");
}
