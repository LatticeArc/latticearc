...
pub use types::{
    CryptoContext, CryptoPayload, CryptoScheme, EncryptedData, EncryptedMetadata, HashOutput,
    KeyPair, PerformancePreference, PrivateKey, PublicKey, SecurityLevel, SignedData,
    SignedMetadata, SymmetricKey, UseCase, ZeroizedBytes,
};
pub use zero_trust::{
...
// Verified API (requires VerifiedSession)
pub use convenience::{
    // Hybrid encryption
    ...
    decrypt_with_config,
    derive_key,
    derive_key_with_config,
    // Core API
    encrypt,
    // AES-GCM
    encrypt_aes_gcm,
    encrypt_aes_gcm_with_config,
    encrypt_for_use_case,
    encrypt_hybrid,
...
// Unverified API (opt-out for scenarios without session management)
pub use convenience::{
    ...
    encrypt_unverified,
    encrypt_with_config_unverified,
