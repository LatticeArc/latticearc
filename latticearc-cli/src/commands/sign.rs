...
//!
//! Supports 7 signing algorithms:
//! - **ML-DSA-44/65/87** (FIPS 204) — post-quantum lattice-based signatures
//! - **SLH-DSA-SHAKE-128s** (FIPS 205) — post-quantum hash-based (stateless)
//! - **FN-DSA-512** (FIPS 206 draft) — post-quantum lattice (compact signatures)
//! - **Ed25519** (RFC 8032) — classical elliptic curve
//! - **Hybrid** — dual ML-DSA-65 + Ed25519 (both must verify)
//!
//! The signing flow:
//! 1. Read the input file into memory
//! 2. Load and validate the secret key file (must match algorithm)
//! 3. Compute the signature using the `latticearc` library
//! 4. Write the signature as a JSON file with Base64-encoded bytes
//!
//! Signature output format:
//! ```json
//! { "algorithm": "ml-dsa-65", "signature": "Base64..." }
//! ```
//!
//! For hybrid signatures, both components are stored separately:
//! ```json
//! { "algorithm": "hybrid-ml-dsa-65-ed25519",
//!   "ml_dsa_sig": "Base64...", "ed25519_sig": "Base64..." }
//! ```
