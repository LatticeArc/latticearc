...
impl TlsConfig {
    /// Create classic TLS configuration (ECDHE only)
    ///
    /// # Example
    /// ```
    ...
    #[must_use]
    pub fn classic() -> Self {
        Self { mode: TlsMode::Classic, enable_fallback: false, ..Default::default() }
    }

    /// Create hybrid TLS configuration (default, ECDHE + ML-KEM)
    ///
    /// # Example
    /// ```
    /// use arc_tls::TlsConfig;
    ...
    #[must_use]
    pub fn hybrid() -> Self {
        Self { mode: TlsMode::Hybrid, ..Default::default() }
    }

    /// Create post-quantum TLS configuration (ML-KEM only)
    ///
    /// # Example
    /// ```
    /// use arc_tls::TlsConfig;
    ...
    #[must_use]
    pub fn pq() -> Self {
        Self { mode: TlsMode::Pq, ..Default::default() }
    }
    ...
    /// Auto-select TLS mode based on defaults
    ///
    /// Returns Hybrid mode if PQ is available, Classic otherwise.
    /// This is the recommended way to create a config when you don't
    ...
    /// // Will use Hybrid if PQ available, Classic otherwise
    /// ```
    #[must_use]
    pub fn auto() -> Self {
        let ctx = TlsContext::default();
        TlsPolicyEngine::create_config(&ctx)
    }

    /// Create configuration for a specific use case
    ///
    /// Each use case has a recommended TLS mode optimized for its
    /// specific requirements (security, performance, compatibility).
    ///
    /// # Example
    /// ```
    /// use arc_tls::{TlsConfig, TlsUseCase};
    ///
    /// // Financial services: Hybrid for compliance + PQ protection
    /// let config = TlsConfig::for_use_case(TlsUseCase::FinancialServices);
    ///
    /// // IoT devices: Classic for resource constraints
    /// let config = TlsConfig::for_use_case(TlsUseCase::IoT);
    ///
    /// // Government: PQ-only for maximum security
    /// let config = TlsConfig::for_use_case(TlsUseCase::Government);
    /// ```
    #[must_use]
    pub fn for_use_case(use_case: TlsUseCase) -> Self {
        let ctx = TlsContext::with_use_case(use_case);
        TlsPolicyEngine::create_config(&ctx)
    }
    ...
    ///
    /// let config = TlsConfig::with_security_level(SecurityLevel::Maximum);
    /// ```
    #[must_use]
    pub fn with_security_level(level: arc_core::SecurityLevel) -> Self {
        let ctx = TlsContext::with_security_level(level);
        TlsPolicyEngine::create_config(&ctx)
    }

    /// Create configuration from full context
    ///
    /// Use this when you need fine-grained control over the
    /// selection process, including constraints and preferences.
    ///
    /// # Example
    /// ```
    /// use arc_tls::{TlsConfig, TlsContext, TlsUseCase, TlsConstraints};
    /// use arc_core::{SecurityLevel, PerformancePreference};
    ///
    /// let ctx = TlsContext::default()
    ///     .security_level(SecurityLevel::High)
    ///     .performance_preference(PerformancePreference::Speed)
    ///     .use_case(TlsUseCase::ApiGateway);
    ///
    /// let config = TlsConfig::from_context(&ctx);
    /// ```
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn test_tls_config_classic() {
        let config = TlsConfig::classic();
        assert_eq!(config.mode, TlsMode::Classic);
    }

    #[test]
    fn test_tls_config_hybrid() {
        let config = TlsConfig::hybrid();
        assert_eq!(config.mode, TlsMode::Hybrid);
    }

    #[test]
    fn test_tls_config_pq() {
        let config = TlsConfig::pq();
    ...
    fn test_tls_config_with_tracing() {
        let config = TlsConfig::default().with_tracing();
    ...
    fn test_tls_config_with_retry() {
        let policy = RetryPolicy::default();
        let config = TlsConfig::default().with_retry_policy(policy);
        assert!(config.retry_policy.is_some());
    }

    #[test]
    fn test_tls_config_with_fallback() {
        let config = TlsConfig::default().with_fallback(false);
        assert!(!config.enable_fallback);
    }

    #[test]
    fn test_tls13_config_from_tls_config() {
        let tls_config = TlsConfig::hybrid();
    ...
    #[test]
    fn test_tls_config_auto() {
        let config = TlsConfig::auto();
        // Auto should select Hybrid (PQ is always available)
        assert_eq!(config.mode, TlsMode::Hybrid);
    }

    #[test]
    fn test_tls_config_for_use_case_webserver() {
        let config = TlsConfig::for_use_case(TlsUseCase::WebServer);
        assert_eq!(config.mode, TlsMode::Hybrid);
    }

    #[test]
    fn test_tls_config_for_use_case_iot() {
        let config = TlsConfig::for_use_case(TlsUseCase::IoT);
        assert_eq!(config.mode, TlsMode::Classic);
    }

    #[test]
    fn test_tls_config_for_use_case_government() {
        let config = TlsConfig::for_use_case(TlsUseCase::Government);
        assert_eq!(config.mode, TlsMode::Pq);
    }

    #[test]
    fn test_tls_config_with_security_level() {
        use arc_core::SecurityLevel;

        let config = TlsConfig::with_security_level(SecurityLevel::Maximum);
        assert_eq!(config.mode, TlsMode::Pq);

        let config = TlsConfig::with_security_level(SecurityLevel::Low);
        assert_eq!(config.mode, TlsMode::Classic);
    }

    #[test]
    fn test_tls_config_from_context() {
