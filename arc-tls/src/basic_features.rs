...
/// # Example
/// ```no_run
/// use arc_tls::{TlsConfig, basic_features::create_client_connector};
///
/// // Default: hybrid mode with PQ key exchange
/// let connector = create_client_connector(&TlsConfig::default())?;
///
/// // Classic mode (no PQ)
/// let classic_connector = create_client_connector(&TlsConfig::classic())?;
///
/// // mTLS: client presents certificate
/// let mtls_connector = create_client_connector(
///     &TlsConfig::hybrid().with_client_auth("client.crt", "client.key")
/// )?;
/// # Ok::<(), arc_tls::TlsError>(())
/// ```
