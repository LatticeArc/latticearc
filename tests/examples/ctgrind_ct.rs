...
//!
//! Constraints:
//! - **Must run under Valgrind on Linux.** The `crabgrind` dep (and
//!   therefore this harness) is gated on `cfg(target_os = "linux")`;
//!   on Windows/macOS the example compiles to a stub that prints a
...
mod harness {
    ...
    fn mark_secret(bytes: &[u8]) {
        // `mark_memory` returns `Err(UnaddressableBytes)` if the range
        // is unaddressable. For stack- or heap-backed `&[u8]` it always
    ...
    //
    // Establishes the baseline — if subtle itself leaks, every caller
    // is suspect. `subtle` uses XOR + OR accumulation and `black_box`
    // to prevent the optimizer from inserting branches; valgrind
    // should see no branch on the marked bytes.
    fn check_subtle_ct_eq_equal() {
        let a = [0x42u8; 32];
        let b = [0x42u8; 32];
        mark_secret(&a);
        mark_secret(&b);
        // `black_box` prevents the compiler from folding the
        // known-equal pair into a constant `Choice::from(1)`.
    ...
    //
    // Exercises the ct_eq impl landed in bfeb9b0e end-to-end: the
    // level comparison (public parameter, not marked), the ML-KEM
    // byte compare, and the ECDH seed compare. Both sides equal =
    // single-iteration of the all-legs-true path.
    fn check_hybrid_ct_eq_equal() {
        ...
        // Mark the byte-level secrets as Undefined. (security_level
        // is not marked — it is a public parameter, and its compare
...
fn main() {
    ...
    {
        println!(
            "ctgrind harness is Linux-only (requires Valgrind). \
             Run via .github/workflows/ctgrind.yml."
        );
    }
}
