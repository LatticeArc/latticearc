#![allow(clippy::unwrap_used, clippy::redundant_clone, clippy::clone_on_copy, clippy::single_match)]

//! Integration tests for the unified signing API:
//! `generate_signing_keypair()` + `sign_with_key()` + `verify()`

use latticearc::unified_api::{
    CryptoConfig, SecurityLevel, generate_signing_keypair, sign_with_key, verify,
};
